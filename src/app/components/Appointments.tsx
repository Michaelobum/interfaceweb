...
// ─── Config ───────────────────────────────────────────────────────────────────
const STATUS_CFG = {
  confirmed: { label: 'Confirmada', bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', dot: 'bg-emerald-500', bar: 'bg-emerald-400', icon: CheckCircle2 },
  pending:   { label: 'Pendiente',  bg: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-200',   dot: 'bg-amber-400',   bar: 'bg-amber-400',   icon: AlertCircle  },
  cancelled: { label: 'Cancelada',  bg: 'bg-red-50',     text: 'text-red-700',     border: 'border-red-200',     dot: 'bg-red-400',     bar: 'bg-red-400',     icon: XCircle      },
  completed: { label: 'Atendida',   bg: 'bg-blue-50',    text: 'text-blue-700',    border: 'border-blue-200',    dot: 'bg-blue-400',    bar: 'bg-blue-500',    icon: CheckCircle2 },
} as const;
...
const inputCls = 'w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#0066CC]/30 focus:border-[#0066CC] transition-all';
    ...
    <div className="min-h-screen bg-gray-50/60">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">Citas</h1>
            <p className="text-sm text-gray-400 mt-0.5">Agenda y programación de pacientes</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          {([
            { key: 'confirmed', label: 'Confirmadas', val: stats.confirmed, cls: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-gray-100' },
            { key: 'pending',   label: 'Pendientes',  val: stats.pending,   cls: 'text-amber-500',   bg: 'bg-amber-50',   border: 'border-amber-100' },
            { key: 'cancelled', label: 'Canceladas',  val: stats.cancelled, cls: 'text-red-500',     bg: 'bg-red-50',     border: 'border-red-100' },
          ] as const).map(({ key, label, val, cls, bg, border }) => (
            <div key={key} className={`bg-white rounded-2xl p-4 border ${border} shadow-sm`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {(['all', 'confirmed', 'pending', 'cancelled', 'completed'] as const).map(s => (
                <button key={s} onClick={() => setFilterStatus(s)}
                  className={`px-3 py-2 rounded-xl text-xs font-semibold transition-all ${
                    filterStatus === s ? 'bg-[#0066CC] text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                  }`}>
              ...
              {/* View toggle */}
              <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl ml-auto sm:ml-0">
                {([
        ...
        {view !== 'list' && (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <button onClick={prevPeriod} className="p-2 hover:bg-white hover:shadow-sm rounded-xl border border-transparent hover:border-gray-100 transition-all">
                <ChevronLeft className="w-4 h-4 text-gray-600" />
              </button>
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-2 min-w-[180px] text-center">
              ...
              <button onClick={nextPeriod} className="p-2 hover:bg-white hover:shadow-sm rounded-xl border border-transparent hover:border-gray-100 transition-all">
                <ChevronRight className="w-4 h-4 text-gray-600" />
              </button>
            </div>
            <button onClick={() => setCurrentDate(TODAY)}
              className="px-3 py-1.5 text-xs font-semibold text-[#0066CC] bg-blue-50 border border-blue-100 rounded-xl hover:bg-blue-100 transition-colors">
              Hoy
            </button>
          </div>
        )}
              ...
              <div style={{ minWidth: 700 }}>
                ...
                <div className="grid border-b border-gray-100" style={{ gridTemplateColumns: '56px repeat(7, 1fr)' }}>
                  ...
                  {weekDays.map((d, i) => {
                    ...
                    return (
                      <div key={i} className={`p-3 text-center border-l border-gray-100 cursor-pointer hover:bg-blue-50/40 transition-colors ${isToday ? 'bg-blue-50/60' : 'bg-gray-50/40'}`}
                        onClick={() => { setCurrentDate(d); setView('day'); }}>
                        <p className={`text-[11px] font-semibold uppercase tracking-wider ${isToday ? 'text-[#0066CC]' : 'text-gray-400'}`}>
                          {format(d, 'EEE', { locale: es })}
                        </p>
                        <p className={`text-lg font-bold mt-0.5 ${isToday ? 'text-[#0066CC]' : 'text-gray-700'}`}>
                          {format(d, 'd')}
                        </p>
                        {count > 0 && (
                ...
                <div style={{ maxHeight: 480, overflowY: 'auto' }}>
                  {HOURS.map(hour => (
                    <div key={hour} className="grid border-b border-gray-50 last:border-0" style={{ gridTemplateColumns: '56px repeat(7, 1fr)', minHeight: 80 }}>
                      <div className="bg-gray-50/40 px-2 py-2 text-[10px] font-semibold text-gray-400 text-right pr-3 pt-2">
                      ...
                      {weekDays.map((d, di) => {
                        ...
                        const isToday = isSameDay(d, TODAY);
                        return (
                          <div key={di} className={`border-l border-gray-100 relative ${isToday ? 'bg-blue-50/20' : 'hover:bg-gray-50/60'} transition-colors`}
                            style={{ minHeight: 80 }}>
                            {apts.map(apt => (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div style={{ maxHeight: 520, overflowY: 'auto' }}>
              {HOURS.map(hour => {
                ...
                return (
                  <div key={hour} className="flex gap-0 border-b border-gray-50 last:border-0" style={{ minHeight: 80 }}>
                    <div className="w-14 shrink-0 bg-gray-50/60 px-2 pt-2 text-[10px] font-semibold text-gray-400 text-right">
                      {String(hour).padStart(2, '0')}:00
                    </div>
                    <div className="flex-1 relative border-l border-gray-100">
                      {apts.length === 0 && (
                        <div className="absolute inset-0 hover:bg-blue-50/30 transition-colors cursor-pointer" />
                      )}
                      {apts.map(apt => (
                        <div key={apt.id} className="absolute left-2 right-2"
                          ...
                          <div className={`h-full rounded-xl border-l-4 px-3 py-1.5 cursor-pointer hover:shadow-md transition-shadow flex items-center gap-3`}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                <span className="text-xs font-bold text-gray-500">{apt.time}</span>
                                <StatusBadge status={apt.status} />
                              </div>
                              <p className="text-sm font-bold text-gray-900 mt-0.5 truncate">{apt.patient}</p>
                              <p className="text-xs text-gray-500 truncate">{apt.treatment} · {apt.doctor}</p>
                            </div>
                            <span className="text-xs text-gray-400 shrink-0">{apt.duration}min</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <thead>
                  <tr className="border-b border-gray-100">
                    {['Fecha', 'Hora', 'Paciente', 'Tratamiento', 'Doctor', 'Duración', 'Estado'].map((h, i) => (
                      <th key={h} className={`px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wider ${i === 6 ? 'text-center' : 'text-left'}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {filteredList.map(apt => (
                    <tr key={apt.id} onClick={() => setSelectedApt(apt)}
                      ...
                      <td className="px-5 py-4 text-sm text-gray-500 capitalize">
                        ...
                        {isSameDay(apt.date, TODAY) && <span className="ml-1 text-[10px] font-bold text-[#0066CC] bg-blue-50 px-1.5 py-0.5 rounded-full">Hoy</span>}
                      </td>
                      <td className="px-5 py-4 text-sm font-semibold text-gray-800">{apt.time}</td>
                      ...
                      <td className="px-5 py-4 text-sm text-gray-600">{apt.treatment}</td>
                      <td className="px-5 py-4 text-sm text-gray-500">{apt.doctor}</td>
                      <td className="px-5 py-4 text-sm text-gray-400">{apt.duration} min</td>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">{apt.treatment}</p>
                      </div>
                    </div>
                    <StatusBadge status={apt.status} />
                  </div>
      ...
      {selectedApt && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
          ...
          <div className="bg-white w-full sm:max-w-md sm:rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()} style={{ maxHeight: '85vh', overflowY: 'auto' }}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <p className="text-sm font-bold text-gray-900">Detalle de Cita</p>
              <button onClick={() => setSelectedApt(null)} className="p-1.5 hover:bg-gray-100 rounded-xl transition-colors">
            ...
            <div className="px-5 py-5 space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-base font-bold text-gray-900">{selectedApt.patient}</p>
                  {selectedApt.phone && <p className="text-xs text-gray-400 mt-0.5">{selectedApt.phone}</p>}
                </div>
              ...
              {selectedApt.notes && (
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
                  <p className="text-xs font-semibold text-amber-600 mb-1">Notas</p>
                  <p className="text-sm text-amber-800">{selectedApt.notes}</p>
                </div>
              )}

              <div className="flex gap-2 pt-1">
                <button className="flex-1 py-2.5 text-sm font-semibold border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 transition-colors">
                  Cancelar cita
                </button>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
          ...
          <div className="bg-white w-full sm:max-w-lg sm:rounded-2xl shadow-2xl flex flex-col" style={{ maxHeight: '95vh' }}
            onClick={e => e.stopPropagation()}>

            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
              <div>
                <h2 className="text-base font-bold text-gray-900">Nueva Cita</h2>
                <p className="text-xs text-gray-400">Completa los datos de la cita</p>
              </div>
            ...
            <div className="overflow-y-auto flex-1 px-5 py-5 space-y-4">
                  ...
                  {[30, 45, 60, 90, 120].map(d => (
                    <button key={d} onClick={() => setNewApt({ ...newApt, duration: String(d) })}
                      className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition-all ${
                        newApt.duration === String(d) ? 'bg-[#0066CC] text-white border-[#0066CC]' : 'border-gray-200 text-gray-500 hover:border-gray-300'
                      }`}>
                      {d}min
                    </button>
                  ))}
              ...
              <label className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl border border-blue-100 cursor-pointer">
                ...
                <div>
                  <p className="text-sm font-semibold text-blue-800">Enviar recordatorio al paciente</p>
