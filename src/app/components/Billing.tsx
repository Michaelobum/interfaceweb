...
const STATUS_CONFIG = {
  paid:      { label: 'Pagada',    bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', dot: 'bg-emerald-500', icon: CheckCircle2 },
  pending:   { label: 'Pendiente', bg: 'bg-amber-50',   text: 'text-amber-700',   border: 'border-amber-200',   dot: 'bg-amber-400',   icon: Clock },
  overdue:   { label: 'Vencida',   bg: 'bg-red-50',     text: 'text-red-700',     border: 'border-red-200',     dot: 'bg-red-500',     icon: AlertCircle },
  cancelled: { label: 'Cancelada', bg: 'bg-gray-50',    text: 'text-gray-500',    border: 'border-gray-200',    dot: 'bg-gray-400',    icon: X },
};
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Gestión de facturas y cobros</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
            ...
            <p className="text-xl font-bold text-gray-900">${totalPaid.toFixed(0)}</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-red-100 shadow-sm">
            ...
            <p className="text-xs text-gray-500 mb-0.5">Vencido</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Factura</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Paciente</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider hidden lg:table-cell">Emisión</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider hidden lg:table-cell">Vencimiento</th>
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-sm font-medium text-gray-800">{invoice.patient}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-5 py-4 hidden lg:table-cell">
                      <span className={`text-sm ${invoice.status === 'overdue' ? 'text-red-600 font-semibold' : 'text-gray-500'}`}>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="p-2 rounded-lg hover:bg-blue-100 text-gray-400 hover:text-[#0066CC] transition-colors"
                          title="Ver detalle"
                        >
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-gray-900 truncate">{invoice.patient}</p>
          ...
          <div className="bg-white w-full sm:rounded-2xl sm:max-w-2xl flex flex-col shadow-2xl" style={{ maxHeight: '95vh' }}>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-medium">Factura</p>
                  <p className="text-sm font-bold text-gray-900">{selectedInvoice.invoiceNumber}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <StatusBadge status={selectedInvoice.status} />
                <button onClick={() => setShowModal(false)} className="p-2 hover:bg-gray-100 rounded-xl transition-colors ml-1">
            ...
            <div className="overflow-y-auto flex-1 px-5 py-5 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-sm text-gray-500 mt-0.5">ID: {selectedInvoice.patientId}</p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
                  ...
                  <div className="space-y-1 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-gray-800">{format(selectedInvoice.date, "dd MMM yyyy", { locale: es })}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-500">Vencimiento</span>
                      <span className={`font-medium ${selectedInvoice.status === 'overdue' ? 'text-red-600' : 'text-gray-800'}`}>
                    ...
                    {selectedInvoice.paymentMethod && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-gray-800">{selectedInvoice.paymentMethod}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Items */}
              <div>
                <p className="text-sm font-bold text-gray-800 mb-3">Conceptos</p>
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">IVA</th>
                    ...
                    <tbody className="divide-y divide-gray-50">
                      {selectedInvoice.items.map(item => (
                        <tr key={item.id}>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-500 text-right">{item.tax}%</td>
                    ...
                    <div key={item.id} className="bg-gray-50 rounded-xl p-3.5">
                      ...
                      <div className="grid grid-cols-3 gap-2 text-xs">
                        <div className="text-center">
                          <p className="text-gray-400 mb-0.5">Cantidad</p>
                        ...
                        <div className="text-center">
                          <p className="text-gray-400 mb-0.5">IVA</p>
                          <p className="font-semibold text-gray-800">{item.tax}%</p>
                        </div>
                      </div>
                      <div className="mt-2.5 pt-2.5 border-t border-gray-200 flex justify-between items-center">
                        <span className="text-xs text-gray-400">Total línea</span>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-gray-800">${selectedInvoice.subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">IVA (21%)</span>
                  <span className="font-medium text-gray-800">${selectedInvoice.tax.toFixed(2)}</span>
                </div>
            ...
            <div className="px-5 py-4 border-t border-gray-100 flex flex-col-reverse sm:flex-row gap-2.5 shrink-0">
              <button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 border border-gray-200 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-50 transition-colors"
              >
                <Download className="w-4 h-4" />
                Descargar PDF
              </button>
          ...
          <div className="bg-white w-full sm:rounded-2xl sm:max-w-3xl flex flex-col shadow-2xl" style={{ maxHeight: '95vh' }}>
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
              <div>
                ...
                <p className="text-xs text-gray-400">Completa los datos del servicio</p>
              </div>
              <button onClick={() => setShowNewInvoiceModal(false)} className="p-2 hover:bg-gray-100 rounded-xl transition-colors">
                <X className="w-4 h-4 text-gray-400" />
              </button>
            </div>
            ...
            <div className="overflow-y-auto flex-1 px-5 py-5 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Fecha de emisión *</label>
                  <input type="date" defaultValue={format(new Date(), 'yyyy-MM-dd')} className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#0066CC]/30 focus:border-[#0066CC] transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">Fecha de vencimiento *</label>
                  <input type="date" className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#0066CC]/30 focus:border-[#0066CC] transition-all" />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-bold text-gray-800">Conceptos</p>
                  <button
                    type="button"
                    onClick={addInvoiceItem}
                    className="text-xs font-semibold text-[#0066CC] hover:bg-blue-50 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"
                  >
                    ...
                    <div key={item.id} className="bg-gray-50 rounded-xl p-3.5 space-y-3">
                      ...
                      <div className="flex items-end gap-2">
                        <div className="flex-1">
                          ...
                          <input type="number" value={item.quantity} min="1" onChange={e => updateInvoiceItem(item.id, 'quantity', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#0066CC]/30 focus:border-[#0066CC] transition-all" />
                        </div>
                        <div className="flex-1">
                          <label className="block text-xs text-gray-400 mb-1">Precio</label>
                          <input type="number" value={item.unitPrice} min="0" step="0.01" onChange={e => updateInvoiceItem(item.id, 'unitPrice', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#0066CC]/30 focus:border-[#0066CC] transition-all" />
                        </div>
                        <div className="flex-1">
                          <label className="block text-xs text-gray-400 mb-1">IVA %</label>
                          <input type="number" value={item.tax} min="0" max="100" onChange={e => updateInvoiceItem(item.id, 'tax', parseFloat(e.target.value) || 0)} className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#0066CC]/30 focus:border-[#0066CC] transition-all" />
                        </div>
                        {invoiceItems.length > 1 && (
                          <button type="button" onClick={() => removeInvoiceItem(item.id)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg transition-colors self-end">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Subtotal</span>
                  <span className="font-medium text-gray-800">${calcTotals().subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">IVA</span>
                  <span className="font-medium text-gray-800">${calcTotals().tax.toFixed(2)}</span>
                </div>
              ...
              <div>
                ...
                <textarea rows={3} placeholder="Añade notas o comentarios sobre esta factura…" className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-[#0066CC]/30 focus:border-[#0066CC] transition-all resize-none" />
              </div>
            </div>
